Shader "Stylized/Sky_ManualRotation_SunControl"
{
    Properties
    {
        [Header(Sun Disc)]
        ...
        [Header(Sun Direction Controls)]
        _SunAzimuth ("Sun Horizontal (degrees)", Range(0, 360)) = 0
        _SunElevation ("Sun Vertical (degrees)", Range(-90, 90)) = 15
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                // ✅ Use object space (respects sphere rotation)
            ...
            {
                float3 localDir = normalize(i.localDir);

                // --- Compute sun direction from azimuth/elevation ---
                float azRad = radians(_SunAzimuth);
                float elRad = radians(_SunElevation);
                float3 sunDir;
                sunDir.x = cos(elRad) * sin(azRad);
                sunDir.y = sin(elRad);
                sunDir.z = cos(elRad) * cos(azRad);

                // --- Horizon & sun masks ---
                float maskHorizon = dot(localDir, float3(0, 1, 0));
                float maskSunDir = dot(localDir, sunDir);
