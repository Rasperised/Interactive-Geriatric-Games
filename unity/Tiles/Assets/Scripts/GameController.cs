...
public class GameController : MonoBehaviour
{
    ...
    // Name of image chosen from Resources/Images
    private string selectedResourceImage = null;
    ...
    public void OnTakePhotoClicked()
    {
        ...
        // 🔧 FIX: clear previous image sources
    ...
    public void OnUsePhoto()
    {
        ...
        // 🔧 FIX: ensure camera overrides everything else
        selectedResourceImage = null;
        forcedImagePath = null;
    ...
    void LoadThumbnails()
    {
        ...
        Texture2D[] images = Resources.LoadAll<Texture2D>("Images");

        if (images == null || images.Length == 0)
        {
            Debug.LogWarning("No images found in Resources/Images");
            return;
        }
        ...
        foreach (Texture2D tex in images)
        {
            ...
            if (b != null)
            {
                string imageName = tex.name;
                b.onClick.AddListener(() =>
                {
                    StartPuzzleWithResourceImage(imageName);
                });
            }
        }
    }

    void StartPuzzleWithResourceImage(string resourceName)
    {
        forcedImagePath = null; // important
        selectedResourceImage = resourceName;
    ...
    {
        // 1️⃣ Resource-selected image
        if (!string.IsNullOrEmpty(selectedResourceImage))
        {
            Texture2D tex = Resources.Load<Texture2D>("Images/" + selectedResourceImage);
            if (tex != null)
                return tex;
        }
        ...
        // 4️⃣ Fallback: first image in Resources
        Texture2D[] res = Resources.LoadAll<Texture2D>("Images");
        if (res.Length > 0)
            return res[0];

        Debug.LogError("No textures available.");
        return null;
    }
